...
Matrix multiplyMatricesParallel(Matrix matrix1, Matrix matrix2) {
    ...
    int i, j, k;
        ...
        for (i  = 0; i < matrix1.rows; i++) {
		    ...
		    result[getIndex(matrix2, i, j)] = 0;
                for (k = 0; k < matrix2.rows; k++) {
                    result[getIndex(matrix2, i, j)] += matrix1.table[getIndex(matrix1, i, k)] * matrix2.table[getIndex(matrix2, k, j)];
                }
            }
        }
...
Vector multiplyMatVectParallel(Matrix matrix, Vector vector) {
    ...
    double* matrixTable = matrix.table;
    double* vectorTable = vector.table;
    ...
    for (int i = 0; i < numRows; i++) {
        ...
        for (int j = 0; j < numCols; j++) {
            int matrixIndex = rowStart + j;
            double cellResult;
            #pragma omp critical
            {
                cellResult = matrixTable[matrixIndex] * vectorTable[j];
            }

            rowResult += cellResult;
        }
...
void print_result(float guess,  int duration, bool parse = false){
    // if (duration > 1000000 && parse == true ){
    //     int seconds = floor(duration/1000000);
    //     printf("Current result: %.8f.\nTime taken: %d seconds\n", guess, seconds);
    // }
    // else if (duration > 1000 && parse == true){
...
void test_time_func(float (*func)(int), int n){
    // start timer
    auto startTime = high_resolution_clock::now();
    float result = func(n);
    auto stopTime = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(stopTime - startTime);
    print_result(result, duration.count());
}
